...
class Enforcer
{
     ...
     * @var String
     */
    protected static $example = "default";
    ...
    public function instance()
    {
        if (!$this->enforcer) {
            ...
            if (!isset($config[self::$example]) || is_null($config)) {
                throw new InvalidArgumentException("Enforcer config is not defined.");
            }
            $config = $config[self::$example];
    ...
    /**
     * Declare current instance
     * @param string $example
     */
    public static function example($example = "default")
    {
        self::$example = $example;
        return ApplicationContext::getContainer()->get(Enforcer::class);
    }
